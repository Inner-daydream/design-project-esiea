            ...
            List<Program> program = programEntities.stream().map(programEntity ->
                    new Program(
                            ...
                            programEntity.getEvents().stream().map(
                                    courseEntity -> {
                                        Optional<CourseEntity> course = postgreCourseDataRepository.findAll().stream().filter(item -> item.getEvent()!=null && item.getEvent().getId().equals(courseEntity.getId())).findFirst();
                                        if(!course.isPresent()){
                                            return null;
                                        }
                                        return new Course(
